...
public interface EmployeeRequestRepository extends JpaRepository<EmployeeRequest, Long> {
    ...
    @Query("SELECT r FROM EmployeeRequest r WHERE r.user.id = :userId AND r.status = 'APPROVED' AND r.startDate >= :startDate AND r.endDate <= :endDate ORDER BY r.startDate DESC")
